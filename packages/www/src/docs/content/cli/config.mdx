...
`turn` shows **Last turn**: the files the session changed since your last prompt, from the start of its first step to the end of its last step, or to the working copy while the turn is still running. It is only available inside a session and falls back to `branch` elsewhere.
